...
func verifyDNS(dns1, dns2 string) (ok bool, detail string) {
	testHost := "google.com"
	logf("dns-verify: querying %s directly against %s for %s", testHost, dns1, dns1)

	if ok, detail := queryAgainst(dns1, testHost); ok {
		logf("dns-verify: dns1 (%s) answered: %s", dns1, detail)
		return true, fmt.Sprintf("resolved via %s: %s", dns1, detail)
	} else if dns2 != "" {
		logf("dns-verify: dns1 (%s) failed (%s), trying dns2 (%s)", dns1, detail, dns2)
		if ok2, detail2 := queryAgainst(dns2, testHost); ok2 {
			logf("dns-verify: dns2 (%s) answered: %s", dns2, detail2)
			return true, fmt.Sprintf("resolved via %s (dns1 failed): %s", dns2, detail2)
		} else {
			logf("dns-verify: both dns1 and dns2 failed")
			return false, fmt.Sprintf("both failed — dns1(%s): %s | dns2(%s): %s", dns1, detail, dns2, detail2)
		}
	}
	logf("dns-verify: dns1 failed and no dns2 configured")
